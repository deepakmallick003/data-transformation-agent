...
`main.py` does not load `.env` by itself, so for local development the environment variables must be exported into the shell before starting the app.
...
python scripts/deploy_agentcore.py check
